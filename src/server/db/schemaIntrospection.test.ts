...
describe('schema introspection normalization', () => {
  it('normalizes booleans consistently across dialects', () => {
    expect(normalizeSqlType('sqlite', 'INTEGER', 'is_pinned')).toBe('boolean');
    expect(normalizeSqlType('mysql', 'tinyint', 'is_pinned')).toBe('boolean');
    expect(normalizeSqlType('postgres', 'boolean', 'is_pinned')).toBe('boolean');
  });
