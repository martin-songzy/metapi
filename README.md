...
用站点自己的 apikey 拉取模型列表，按正则筛出你关心的模型，然后**发真实请求**逐个验证。中转站常见的做法是 HTTP 200 里包一个错误体，只看状态码的测活会把已死的模型报成可用。

**仅在你手动触发时运行**——每次探测都在消耗账号配额，不会自动跑。

判定分三态，区分「模型不存在」和「账号出了问题」：

| 判定       | 含义               | 副作用           |
| ---------- | ------------------ | ---------------- |
| **可用**   | 返回了协议原生内容 | 无               |
| **不可用** | 命中错误关键词表   | 写入站点禁用模型 |
| **未确定** | 有错误但无法归因   | **不写任何状态** |
